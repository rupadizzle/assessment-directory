    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <section className="mt-12">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">
          Find {guide.condition === "autism" ? "Autism" : guide.condition === "adhd" ? "ADHD" : "ADHD & Autism"} Clinics Near You
        </h2>
        {guide.condition === "both" ? (
          <div className="space-y-4">
            <div>
              <h3 className="text-sm font-medium text-gray-700 mb-2">ADHD Assessment</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {topCities.map((town) => (
                  <Link
                    key={`adhd-${town.slug}`}
                    href={`/adhd-assessment/${town.slug}/`}
                    className="text-sm px-3 py-2 rounded-lg border transition-colors text-blue-700 bg-blue-50 border-blue-100 hover:bg-blue-100"
                  >
                    {town.name}
                  </Link>
                ))}
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-700 mb-2">Autism Assessment</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {topCities.map((town) => (
                  <Link
                    key={`autism-${town.slug}`}
                    href={`/autism-assessment/${town.slug}/`}
                    className="text-sm px-3 py-2 rounded-lg border transition-colors text-purple-700 bg-purple-50 border-purple-100 hover:bg-purple-100"
                  >
                    {town.name}
                  </Link>
                ))}
